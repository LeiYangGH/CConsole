...
void init()
{
	...
	int n;
	n = (rand() % 4) * 2 + 1;
	g_offset_a.x = n / 3 - 1; g_offset_a.y = n % 3 - 1;
	n = (rand() % 4) * 2 + 1;
	g_offset_b.x = n / 3 - 1; g_offset_b.y = n % 3 - 1;
...
int GetCmd()
{
	...
	while (_kbhit())
	{
		...
		case 'W':  case 'w': if (PLAYER_A_CMD.size() < 16)
			PLAYER_A_CMD.push(CMD_A_UP); break;
		case 'S':  case 's': if (PLAYER_A_CMD.size() < 16)
			PLAYER_A_CMD.push(CMD_A_DOWN); break;
		case 'A':  case 'a': if (PLAYER_A_CMD.size() < 16)
			PLAYER_A_CMD.push(CMD_A_LEFT); break;
		case 'D':  case 'd': if (PLAYER_A_CMD.size() < 16)
			PLAYER_A_CMD.push(CMD_A_RIGHT); break;
		case  0:  case 0xE0:
			switch (_getch())
			{
			case 72:  if (PLAYER_B_CMD.size() < 16) PLAYER_B_CMD.push(CMD_B_UP);
				break;
			case 80:  if (PLAYER_B_CMD.size() < 16) PLAYER_B_CMD.push(CMD_B_DOWN);
				break;
			case 75:  if (PLAYER_B_CMD.size() < 16) PLAYER_B_CMD.push(CMD_B_LEFT);
				break;
			case 77:  if (PLAYER_B_CMD.size() < 16) PLAYER_B_CMD.push(CMD_B_RIGHT);
				break;
			}
		}
	}
	...
	while (!PLAYER_A_CMD.empty())
	{
		...
		PLAYER_A_CMD.pop();
		if ((c == CMD_A_UP
			|| c == CMD_A_DOWN) && g_offset_a.x != 0)  break;
	...
	while (!PLAYER_B_CMD.empty())
	{
		...
		PLAYER_B_CMD.pop();
		if ((c == CMD_B_UP
			|| c == CMD_B_DOWN) && g_offset_b.x != 0)  break;
...
bool DealCmd(int cmd)
{
	...
	if ((cmd & CMD_A_RIGHT) && g_offset_a.y != 0)
	{
		g_offset_a.x = 1; g_offset_a.y = 0;
	}
	if ((cmd & CMD_B_UP)
		&& g_offset_b.x != 0) {
		g_offset_b.x = 0;
